...
{
#pragma warning disable S3776 // Cognitive Complexity — flat validation logic, splitting would reduce readability
    public ValidateOptionsResult Validate(string? name, OutboxPublisherOptions options)
    {
        ...
        {
            errors.Add(
                $"PartitionGracePeriodSeconds ({options.PartitionGracePeriodSeconds}) must be strictly greater than " +
        ...
        if (options.MaxRetryCount <= options.CircuitBreakerFailureThreshold)
            errors.Add(
                $"MaxRetryCount ({options.MaxRetryCount}) should be greater than " +
