...
public sealed class OutboxHealthState
{
    ...
    private readonly HashSet<string> _openCircuits = [];
    ...
    public void SetPublishLoopRunning(bool running)
    {
        ...
        if (running)
        {
            ...
            // Reset timestamps so health check doesn't see stale values from previous iteration
            Volatile.Write(ref _lastHeartbeatTicks, 0);
            Volatile.Write(ref _lastPollTicks, 0);
            Volatile.Write(ref _lastSuccessfulPublishTicks, 0);
        }
    }
